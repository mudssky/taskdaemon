...
- [x] `export` / `import` / `transfer` 三个子命令可用
- [x] 全部业务 entity 被覆盖，无遗漏（用 schema 对照测试证明）
- [x] 类型映射表完整并写入文档
- [x] 时间戳在两个方向迁移后值正确（含时区）
- [x] 布尔、JSON、大文本字段迁移后值正确
- [x] 外键与引用完整性在迁移后保持
- [x] PostgreSQL 自增序列迁移后正确复位，后续插入无主键冲突（用测试证明）
- [x] schema 版本不一致时返回 `DBXFER_SCHEMA_MISMATCH`
- [x] 目标库非空时默认拒绝，需显式确认
- [x] 目标库连通性与权限在迁移前被检查
- [x] 输出中有显著的备份提示
- [x] 数据库凭据不进日志
...
- [x] 关键表抽样内容校验通过
- [x] 校验失败输出明确差异报告
- [x] `--dry-run` 打印表与预估记录数，零副作用
- [x] 进度输出可见
- [x] 大数据量不整体读入内存（用大数据集验证）
- [x] SQLite → PostgreSQL 双向迁移各有端到端测试
- [x] 迁移文档完成，含类型映射表与回滚方式
- [x] `pnpm test:go`、`pnpm vet:go`、`pnpm test:go:integration` 全绿
