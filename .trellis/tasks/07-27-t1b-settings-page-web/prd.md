...
- [ ] 设置页路由可达，按 section 分面板
- [ ] 每个面板可独立保存，互不干扰
- [ ] 热生效项保存后提示已生效
- [ ] 需重启项保存后有**持久可见**的重启提示
- [ ] 仅配置文件项为只读展示，附配置文件路径，不显示为禁用输入框
- [ ] 表单按规范分层（schema / defaults / payload mapper 分离）
- [ ] 客户端校验不宽于服务端
- [ ] 服务端字段级错误正确映射到对应表单字段
- [ ] 未保存改动离开页面有提示
- [ ] 无改动时保存按钮禁用
...
- [ ] 「重新设置」入口可用
- [ ] 生成类敏感值只显示一次，有复制按钮
- [ ] 敏感值明文不进 localStorage 与持久缓存（用测试证明）
- [ ] 危险操作清单已列出，均有二次确认对话框并说明后果
- [ ] loading / empty / error 状态齐全
- [ ] 错误展示包含 traceId
- [ ] reload 结果在保存后可见
- [ ] 无任何直接 Wails 全局对象引用
- [ ] Desktop 专属项在浏览器显示禁用态与说明
- [ ] 业务逻辑与 schema 映射有单元测试
- [ ] `pnpm typecheck`、`pnpm lint`、`pnpm --filter @taskdaemon/web test` 全绿
- [ ] 与 T1a 联调通过，mock 与真实接口行为一致
