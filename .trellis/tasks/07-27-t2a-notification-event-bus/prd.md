...
- [x] 事件模型定义完成，事件名与 payload 字段表写入 spec
- [x] 严重级别为有限枚举，非法值在编译期或构造期被拒绝
- [x] `Notifier` 接口定义完成，站内 sink 通过注册方式接入
- [x] 新增一个空 sink 无需修改总线代码即可接入（用测试证明）
- [x] 单 sink 失败不影响其他 sink，不阻塞调度器主流程（用测试证明）
- [x] sink 失败有日志且状态可查询
- [x] 每个 sink 可通过配置独立开关
- [x] 任务成功、失败、超时、取消四类事件能正确产生站内通知
- [x] 通知持久化，已读/未读状态可变更
- [x] 保留策略生效，配置为 0 时不限
- [x] 列表 API 支持分页、已读状态筛选、严重级别筛选
- [x] 未读计数 API 可用
- [x] 单条已读、批量已读、全部已读、删除、清空已读均可用
- [x] 全部错误路径返回 `NOTIFY_*` 稳定错误码与 traceId
- [x] 通知内容不含 token、密码、完整命令行等敏感信息（用测试证明）
- [x] C-2 契约文档已写入 spec，路线图 §9 产物路径已回写
- [x] `pnpm vet:go`、`pnpm typecheck`、`pnpm lint` 全绿；本任务相关 Go 包测试全绿。`pnpm test:go` 仅有 config 包 2 个既有路径比较失败（macOS `/var` vs `/private/var`，与 T2a 无关）
