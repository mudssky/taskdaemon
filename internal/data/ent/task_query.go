...
func (tq *TaskQuery) QueryRuns() *RunQuery {
	query := (&RunClient{config: tq.config}).Query()
...
func (tq *TaskQuery) WithRuns(opts ...func(*RunQuery)) *TaskQuery {
	query := (&RunClient{config: tq.config}).Query()
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withRuns; query != nil {
		if err := tq.loadRuns(ctx, query, nodes,
			func(n *Task) { n.Edges.Runs = []*Run{} },
			func(n *Task, e *Run) { n.Edges.Runs = append(n.Edges.Runs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadRuns(ctx context.Context, query *RunQuery, nodes []*Task, init func(*Task), assign func(*Task, *Run)) error {
	...
	query.Where(predicate.Run(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.RunsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.task_runs
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_runs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_runs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
