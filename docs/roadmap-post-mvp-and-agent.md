...
| C-2 | **通知事件 schema**（事件名空间、payload 字段、`Notifier` 接口、sink 注册与失败语义、已读状态模型） | **T2a** | `internal/notify/event.go` + spec 文档 | **W2**、T4、D2、G6 |
...
| 2026-07-27 | v4.1 | G0 实测回写：§5.1 填入 Pi 0.82.0 / OMP 17.1.3；确认 OMP 可 RPC 程序驱动、双 adapter 可抽象；CLI 冷启动均为 high（需预热池）；gateway 薄索引持久化；§7.5 G0=done。详情 `.trellis/tasks/07-27-g0-agent-runtime-spike/research/HANDOFF.md` | G0 worker |
