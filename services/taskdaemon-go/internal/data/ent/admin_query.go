...
func (aq *AdminQuery) QuerySessions() *SessionQuery {
...
func (aq *AdminQuery) WithSessions(opts ...func(*SessionQuery)) *AdminQuery {
	query := (&SessionClient{config: aq.config}).Query()
...
//
//	client.Admin.Query().
//		GroupBy(admin.FieldSingletonKey).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Admin.Query().
//		Select(admin.FieldSingletonKey).
//		Scan(ctx, &v)
...
func (aq *AdminQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Admin, error) {
	...
	if query := aq.withSessions; query != nil {
		if err := aq.loadSessions(ctx, query, nodes,
			func(n *Admin) { n.Edges.Sessions = []*Session{} },
			func(n *Admin, e *Session) { n.Edges.Sessions = append(n.Edges.Sessions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AdminQuery) loadSessions(ctx context.Context, query *SessionQuery, nodes []*Admin, init func(*Admin), assign func(*Admin, *Session)) error {
	...
	query.Where(predicate.Session(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(admin.SessionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.admin_sessions
		if fk == nil {
			return fmt.Errorf(`foreign-key "admin_sessions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "admin_sessions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
