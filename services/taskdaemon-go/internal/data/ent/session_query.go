...
func (sq *SessionQuery) QueryAdmin() *AdminQuery {
...
func (sq *SessionQuery) WithAdmin(opts ...func(*AdminQuery)) *SessionQuery {
	query := (&AdminClient{config: sq.config}).Query()
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	...
	if query := sq.withAdmin; query != nil {
		if err := sq.loadAdmin(ctx, query, nodes, nil,
			func(n *Session, e *Admin) { n.Edges.Admin = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadAdmin(ctx context.Context, query *AdminQuery, nodes []*Session, init func(*Session), assign func(*Session, *Admin)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "admin_sessions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
