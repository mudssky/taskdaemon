...
func (service *Service) CreateTask(ctx context.Context, input CreateTaskInput) (*ent.Task, error) {
	...
	now := service.now()
	return service.store.Client().Task.Create().
		SetName(input.Name).
		SetDescription(input.Description).
		SetEnabled(enabled).
		SetCronExpression(strings.TrimSpace(input.CronExpression)).
		SetTimezone(normalizeTimezone(input.Timezone)).
		SetRunnerType(enttask.RunnerType(input.Runner.Type)).
		SetTimeoutSeconds(timeoutSeconds).
		SetOverlapPolicy(enttask.OverlapPolicySkip).
		SetUpdatedAt(now).
		SetCreatedAt(now).
		SetRunnerConfig(withCronWarnings(runnerConfig, cronResult.Warnings)).
		Save(ctx)
}
...
func (service *Service) registerCronTask(taskRecord *ent.Task) error {
	...
	if taskRecord.Timezone != "" && taskRecord.Timezone != "Local" {
		taskRecord.CronExpression = "CRON_TZ=" + taskRecord.Timezone + " " + taskRecord.CronExpression
	}
	job, err := service.scheduler.NewJob(
		gocron.CronJob(taskRecord.CronExpression, validation.WithSeconds),
