...
// TestInitializeAdminCreatesFirstAdmin 验证初始化接口会创建首个管理员账号。
//
// 参数:
//   - t: Go 测试上下文。
//
// 返回值:
//   - 无。测试失败时通过 require 终止。
func TestInitializeAdminCreatesFirstAdmin(t *testing.T) {
	router := NewRouter(Options{Auth: fakeAuthService{
		initialized: auth.AdminAccount{ID: 7, Username: "admin"},
	}})
	body := bytes.NewBufferString(`{"username":"admin","password":"secret"}`)
	...
	require.JSONEq(t, `{"adminId":7,"username":"admin"}`, rec.Body.String())
}
...
type fakeAuthService struct {
	login       auth.LoginResult
	initialized auth.AdminAccount
	authErr     error
	initErr     error
}

// InitializeAdmin 返回预设初始化结果或错误。
//
// 参数:
...
//
// 返回值:
//   - auth.AdminAccount: 预设管理员账号摘要。
//   - error: 预设错误。
func (fake fakeAuthService) InitializeAdmin(_ context.Context, _, _ string) (auth.AdminAccount, error) {
	if fake.initErr != nil {
		return auth.AdminAccount{}, fake.initErr
	}
	return fake.initialized, nil
}
